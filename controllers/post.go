...
func (pc PostController) GetPost(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	// Fetch user
	if err := pc.session.DB("go_rest_tutorial").C("users").FindId(oid).One(&u); err != nil {
		w.WriteHeader(404)
		return
	}
...
func (pc PostController) CreatePost(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	pc.session.DB("go_rest_tutorial").C("users").Insert(u)
...
// RemovePost removes an existing user resource
func (pc PostController) RemovePost(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	// Grab id
	id := p.ByName("id")
	...
	// Remove user
	if err := pc.session.DB("go_rest_tutorial").C("users").RemoveId(oid); err != nil {
		w.WriteHeader(404)
		return
	}
